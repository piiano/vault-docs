...
* PII and PCI DSS information have unique workflows and lifecycles. Vault natively supports these workflows and lifecycles while maximizing data protection, including searching, querying, and processing encrypted and protected information.

* The security and privacy compliance requirements needed when storing PII, PCI DSS, PHI and other types of sensitive information are built into Vault. For example, Vault includes encryption and key rotation, tokenization, tight access controls, full audit, individual privacy rights implementation, and more.
...
For the first time, there's a specialized data store for sensitive personal information. Vault is deployed next to other data stores, so PII and PCI DSS data stays within your account.
